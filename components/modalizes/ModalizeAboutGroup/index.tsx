...
const ModalizeAboutGroup: FC<IProps> = ({ group }) => {
    ...
    if (countThoughts) {
        return (
            <Container>
                <ContainerData entering={FadeInDown.duration(300).delay(300)} onPress={async () => await handleCopy(group)}>
                    <Data>Nome: {group}</Data>
                </ContainerData>
                <ContainerData entering={FadeInDown.duration(300).delay(400)} onPress={async () => await handleCopy(String(countThoughts))}>
